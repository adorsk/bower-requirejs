...
module.exports = function (dep, name, baseUrl) {
  ...
  */
  ...
  var moduleType = dep.pkgMeta.moduleType || [];
  for (var i=0; i < moduleType.length; i++){
    if (moduleType[i] === 'node'){
  ...
  function parsePackage(dep, name, baseUrl){
    ...
    if (dep.pkgMeta.location){
      ...
      if (isAbsolute || hasProtocol){
        ...
        var absoluteLocation = path.join(canonicalDir, dep.pkgMeta.location);
        console.log(absoluteLocation);
        location = getPath(absoluteLocation);
      }
    }
