...
module.exports = function (opts, done) {
   ...
   */
  function writeConfig (bowerConfig) {
    var rjsConfig;
    requirejs.tools.useLib(function (require) {
      rjsConfig = require('transform').modifyConfig(config, function (config) {
        ...
        if (bowerConfig.paths){
          if (config.paths) {
            assign(config.paths, bowerConfig.paths);
          } else {
            config.paths = bowerConfig.paths;
          }
        }
        ...
        if (bowerConfig.packages){
          if (config.packages){
            _.each(bowerConfig.packages, function(bowerPackage){
              config.packages.push(bowerPackage);
            });
          }
          else{
            config.packages = bowerConfig.packages;
          }
        }
